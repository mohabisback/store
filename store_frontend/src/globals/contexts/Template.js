...
export const ExternalReducer = {
  ...
  //state: null,
  getState: () => {
    console.error('ExternalReducer is NOT ready');
  },
  dispatch: () => {
    console.error('ExternalReducer is NOT ready');
  },
...
export const GlobalContextProvider = ({ children }) => {
  ...
  const providedValue = {
    ...
    addToArray1: (item) => {
      dispatch({ type: 'ADD_ARRAY1', payload: item });
    },
    removeFromArray1: (item) => {
      dispatch({ type: 'REM_ARRAY1', payload: item });
    },
    updateObjInArray1byID: (obj) => {
      dispatch({ type: 'UPD_ARRAY1_ID', payload: obj });
    },
    updateObjInArray1byName: (obj) => {
      dispatch({ type: 'UPD_ARRAY1_NAME', payload: obj });
    },
    addToObject1: (property) => {
      dispatch({ type: 'ADD_OBJ1', payload: property });
    },
    removeFromObject1: (propertyKey) => {
      dispatch({ type: 'REM_OBJ1', payload: propertyKey });
    },
    setSignedUser: (signedUser) => {
      dispatch({ type: 'SET_SIGNED_USER', payload: signedUser });
    },
    socketConnect: (socket) => {
      dispatch({ type: 'SOCKET_CONNECT', payload: null });
    },
    setLoading: (loading) => {
  ...
  return <GlobalContext.Provider value={providedValue}>{children}</GlobalContext.Provider>;
};
