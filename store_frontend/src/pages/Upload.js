...
const Upload = () => {
  ...
  const handleSubmit = (e)=>{
    e.preventDefault()
    !fileList ? <></> : Array.from(fileList).forEach((file)=>{
    ...
    switch (select) {
      case '1':
        UploadDS.uploadToServer(formData)
        .then()
        break;
      case '2':
        UploadDS.uploadToMongo(formData)
        .then()
        break;
      default:
        break;
               ...
               onChange={(e)=>{
                setFileList(fileList=>{
                  ...
                  if (fileList && fileList.length){
                    ...
                    oldFilesArr.forEach(file=>{dt.items.add(file)})
                  }
                  const newFilesArr =Array.from(e.target.files)
                  newFilesArr.forEach(file=>{dt.items.add(file)});
                  return dt.files
                })
              }}
        />
