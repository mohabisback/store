...
import { Request, Response } from '../../../interfaces/general';
...
export default class FilesModel {
  ...
  static async download(name: string = '', res: Response, width?: number, height?: number): Promise<void> {
    try {
  ...
  static async saveToServer(name: string = '', absoluteDir: string, width?: number, height?: number): Promise<void> {
    try {
      const bucket = new GridFSBucket(db, {
        bucketName: 'images',
      });
  ...
  static async responseImage(filename: string = '', res: Response, width?: number, height?: number): Promise<boolean> {
    try {
      ...
      downloadStream.on('error',(err)=>{console.log('gridFsBucket download err: ', err)})
      ...
      res.on('error',(err)=>{console.log('res download err: ', err)})

      let pipeline: Sharp | Response;
      if (width) {
        pipeline = sharp();
        pipeline.on('error',(err)=>{console.log('sharp download err: ', err)})
        pipeline.resize(width, height ? height : width, { fit: 'inside' }).pipe(res)
        
        cloneThumb(pipeline, fileNameDimensions(filename, String(width), String(height)) )
        this.uploadFileFromStream(pipeline, fileNameDimensions(filename, String(width), String(height)), foundFiles[0].contentType || '')
      } else {
        pipeline = res;
      }
      downloadStream.pipe(pipeline)
    ...
    try {
      const storage = new GridFsStorage({
        ...
        file: () => {
          ...
          if(contentType.includes('image')) {bucketName = 'images'}
  ...
  static async uploadFileFromFile(filePath: string, newName: string, contentType:string):Promise<boolean> {
    try {
