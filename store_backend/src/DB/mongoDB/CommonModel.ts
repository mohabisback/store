import { Collection, Document, InsertOneResult, UpdateResult, WithId, InsertManyResult, DeleteResult } from 'mongodb';
...
export default class CommonModel {
  ...
  static async AddOne(coll: Collection, doc: any, props?: {}): Promise<number> {
    ...
    if (props) {
      ...
      console.log('checkDuple: ', checkDuple)
      if (checkDuple) {
  ...
  static async getOne(coll: Collection, findProps: {}, projProps: any = {}, refs?: TyRef[]): Promise<Document | null> {
  ...
  static async getMany(
    ...
    page = page ? page - 1 : 0;
    sort = sort ? sort : { id: -1 };
    findProps = {...findProps, id: { $gt: 0 }} //to avoid meta docs.
    let docs: WithId<Document>[];
    try {
      docs = await coll
        .find(findProps, { projection: { _id: 0, ...projProps } })
        .sort(sort)
        .skip(page * limit)
        .limit(limit)
        .toArray();
          ...
          for (let column of Object.keys(facets.buckets)) {
            ...
            facetsAgg[column] = [
              ...
              },
              {
                $match: {
                  count: { $gt: 0 },
                }, //return only buckets with more than 0
              },
            ];
            addFacetProj(column);
          }
