...
export const sqlInsert = (table: string, props: any[], startValues:any[]=[]): {sql:string, values:any[]} => {
  ...
  table = `"${table}"`
  let cols = Object.keys(props[0]).map((k) =>{return ` "${k}"`}).toString();
...
export const sqlDelete = (table: string, findProps: any, limit?: number, startValues:any[]=[]):  {sql:string, values:any[]} => {
  const query ={sql: '', values:startValues}
  table = `"${table}"`
...
export const sqlUpdate = (table: string, findProps: any, setProps: object | string,
  limit?: number, sortProps?: any, startValues:any[]=[]): {sql:string, values:any[]} => {
  ...
  if (typeof setProps === 'string') {
    ...
    //@ts-ignore
    set += Object.keys(setProps).map((k) =>{
        //@ts-ignore
...
export const sqlCount = (table: string, findProps?: any, groupProps?: any,
  sortProps?: any, countOnlySorted = false, startValues:any[]=[]): {sql:string, values:any[]}  => {
...
export const sqlSelectOne = (table: string, findProps: any, projProps?:any, refs?:Ref[]):{sql:string, values:any[]} => {
...
export const sqlSelectSome = (table: string, findProps?: any, projProps?: any,
  limit?: number, skip?: number, sortProps?: any, startValues:any[]=[]):{sql:string, values:any[]} => {
...
export const sqlSelect = (table: string, search?:string, findProps?: any, projProps?: any,
  limit?: number, skip?: number, sortProps?: any, facets?:Facets, startValues:any[]=[]):{sql:string, values:any[]} => {
 //testing
 ...
 if (facets){
   if (facets.singles){
     for (let column of Object.keys(facets.singles)){
       facetsSqlArray.push(`(SELECT '${column}' as facet, "${column}"::text as value, COUNT(*) FROM results_first GROUP BY 2)`)
     }
   }
   if (facets.arrays){
     for (let column of Object.keys(facets.arrays)){
       facetsSqlArray.push(`(SELECT '${column}' as facet, "oneof${column}"::text as value, COUNT(*) FROM results_first, LATERAL unnest("${column}") "oneof${column}" GROUP BY 2)`)
     }
   }
   if (facets.dateTrunks){
     for (let column of Object.keys(facets.dateTrunks)){
       ...
       facetsSqlArray.push(`(SELECT '${column}' as facet, date_trunc('${trunk}', "${column}")::text, COUNT(*) FROM results_first GROUP BY 2)`)
     }
   }
   if (facets.buckets){
     for (let column of Object.keys(facets.buckets)){
       ...
       facetsSqlArray.push(`(SELECT '${column}' as facet, (ARRAY[${buckets.toString()}])[width_bucket("${column}", ARRAY[${buckets.toString()}])]::TEXT as value, COUNT(*) FROM results_first GROUP BY 2)`)
     }
   }
 }
...
const signs = {
  ...
  $gt: '>', gt: '>', $lt: '<', lt: '<',
  $gte: '>=', gte: '>=', $lte: '<=', lte: '<=',
  $in: 'IN', in: 'IN', $nin: 'NOT IN', nin: 'NOT IN',
};
