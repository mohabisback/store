...
if (process.env.ENV?.includes('reset') && !process.env.ENV?.includes('pro')) {
  dbResetOrUp += 'reset';
} else if (process.env.ENV?.includes('dev') || process.env.ENV?.includes('dev')) {
  dbResetOrUp += 'up';
}
...
console.log('dbResetOrUp: ', dbResetOrUp);
