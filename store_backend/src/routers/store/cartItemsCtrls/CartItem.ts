...
import { CartItem, Product } from '../../../interfaces/store';
...
const CartItem = async (req: Request, res: Response, next: NextFunction) => {
  ...
  let cartItems: CartItem[]| undefined = undefined
  try{
    if(!product_id || Number.isNaN(product_id) || Number.isNaN(quantity)){
      message = 'Missing product info.'
    }
    let product: Product|undefined = await ProductsModel.getProduct({id: product_id})
    if (!product) {
  ...
  if (!cartItems){
    cartItems = await CartItemsModel.getSomeCartItems({user_id:req.user.id}, undefined, undefined, undefined, {date:1})
  }
