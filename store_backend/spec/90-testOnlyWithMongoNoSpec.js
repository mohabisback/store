...
describe('Mongo image:', () => {
  beforeAll(async () => {
    if (process.env.ENV.includes('mongo')) {
      ...
      await FilesModel.uploadFileFromFile(
        '../backend/src/assets/images/thumbs/200x200_mongoImage.jpg',
        '200x200_test.jpg',
        'image/jpg',
      );
    }
  ...
  it(
    'creates thumb from full image',
    async () => {
      console.log('it takes a along time, one download, 2 sharp clones, one upload, wait...');
      await request.get(route + 'test.jpg?width=250&height=250').expect(200);
    },
    1000 * 60 * 10,
  );
});
